...
fn select_file_from_list(files: &[String], prompt: &str) -> Result<String> {
    ...
    let file_index: usize = choice.trim().parse::<usize>()
...
fn encrypt_file() -> Result<()> {
    ...
    let full_filename = path_obj.file_name()
        .and_then(|n| n.to_str())
...
fn decrypt_file() -> Result<()> {
    ...
    let filename_with_vault = path_obj.file_name()
        .and_then(|n| n.to_str())
        .ok_or("Invalid filename")?;
    let original_filename = filename_with_vault.strip_suffix(".vault")
